...
class MovieLens_bak(object):
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        for rating in self.possible_rating_values:
            ...
            row_allign.append(rrow)
            col_allign.append(rcol)
            val_allign.append(np.zeros_like(rrow) + rating)
        rating_row = np.concatenate(row_allign, 0)
        rating_col = np.concatenate(col_allign, 0)
        rating_paris = (rating_row, rating_col)
        rating_vals = np.concatenate(val_allign, 0)
...
class DataSetLoader(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        self.train_enc_graph, train_rating_pairs, train_rating_values = self._generate_enc_graph(train_rating_pairs, train_rating_values, add_support=True)
        self.train_dec_graph = self._generate_dec_graph(train_rating_pairs)
        self.train_labels = _make_labels(train_rating_values)
        self.train_truths = th.FloatTensor(train_rating_values).to(device)
        ...
        self.test_enc_graph, all_train_rating_pairs, all_train_rating_values = self._generate_enc_graph(all_train_rating_pairs, all_train_rating_values, add_support=True)
        self.test_dec_graph = self._generate_dec_graph(test_rating_pairs)
        self.test_labels = _make_labels(test_rating_values)
        self.test_truths = th.FloatTensor(test_rating_values).to(device)
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        rating_row = np.concatenate(row_allign, 0)
        rating_col = np.concatenate(col_allign, 0)
        rating_pairs = (rating_row, rating_col)
        rating_values = np.concatenate(val_allign, 0)
        # print(len(data_dict))
        ...
        return graph, rating_pairs, rating_values
